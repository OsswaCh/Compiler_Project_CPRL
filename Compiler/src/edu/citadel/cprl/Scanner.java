...
public final class Scanner
  {
    ...
    private final Map<String, Symbol> rwMap = new HashMap<>(100);
    ...
    public Scanner(File sourceFile, int k, ErrorHandler errorHandler) throws IOException
      {
        ...
        // Initialize the reserved words map
        //fixed: the map needs to be initialized before filling the buffer.
        for (Symbol symbol : Symbol.values()) {
            if (symbol.isReservedWord()) {
                rwMap.put(symbol.toString(), symbol);
            }
        }
...
// ...  Hint: Need an efficient search based on the text of the identifier (parameter idString)
    	...
    	return rwMap.getOrDefault(idString, Symbol.identifier);
    	//done but check
      }
    ...
    private String scanIdentifier() throws IOException
      {
        ...
        // append the leading letter character
        char firstChar = (char) source.currentChar();
        scanBuffer.append(firstChar);
        source.advance();

        while (CharUtil.isLetterOrDigit(source.currentChar()))
          {
...
// ... done but check
    ...
    private void scanHexLiteral() throws ScannerException, IOException
      {
    	assert scanBuffer.charAt(0) == '0' && (scanBuffer.charAt(1) == 'X' || scanBuffer.charAt(1) == 'x')
    	        : "Hexadecimal literal must start with '0x' or '0X'.";

    	    // Check if the next character is a valid hexadecimal digit
    	    if (!CharUtil.isHexDigit((char) source.currentChar())) {
    	        throw error("Improperly formed hex literal: must have at least one hexadecimal digit after '0x' or '0X'.");
    	    }

    	    // Append hexadecimal digits to the scan buffer
    	    do {
    	        scanBuffer.append((char) source.currentChar());
    	        source.advance();
    ...
    private String scanStringLiteral() throws ScannerException, IOException
      {
        ...
        assert source.currentChar() == '\"'; // Ensure we start at a double quote
        
        StringBuilder stringLiteral = new StringBuilder();
        stringLiteral.append((char) source.currentChar());  // append opening quote
        source.advance();
        
        while (source.currentChar() != '"' && source.currentChar() != Source.EOF) {
            checkGraphicChar(source.currentChar());  // This will throw if we hit end of line
            
            if (source.currentChar() == '\\') {
                try {
                    stringLiteral.append(scanEscapedChar());  // Use existing method to handle escapes
                } catch (ScannerException e) {
                    // Report the error but continue scanning
                    errorHandler.reportError(e);
                    // We've already advanced in scanEscapedChar, so continue the loop
                    continue;
                }
            } else {
                stringLiteral.append((char) source.currentChar());
                source.advance();
            }
        }
        ...
        stringLiteral.append((char) source.currentChar());  // append closing quote
        source.advance();
